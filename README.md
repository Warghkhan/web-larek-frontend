...
- **item:add**: Событие, срабатывающее при добавлении товара в корзину. Обновляет состояние корзины и отображает уведомление о добавлении товара.
- **item:remove**: Событие, срабатывающее при удалении товара из корзины. Обновляет состояние корзины и отображает уведомление о удалении товара.
- **inventory:update**: Событие, срабатывающее при обновлении инвентаря. Перезагружает список товаров и обновляет интерфейс.
- **user:login**: Событие, срабатывающее при успешном входе пользователя. Обновляет состояние приложения и отображает информацию о пользователе.
- **user:logout**: Событие, срабатывающее при выходе пользователя. Очищает информацию о пользователе и возвращает приложение в состояние гостя.

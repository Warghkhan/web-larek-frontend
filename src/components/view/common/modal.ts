...
export class Modal extends View<ModalInterface> {
	...
	open() {
		this.container.classList.add('modal_active'); // Добавляем класс для отображения модалки
		this.events.emit('modal:open'); // Генерируем событие открытия модалки
	}

	// Метод для закрытия модального окна
	close() {
		this.container.classList.remove('modal_active'); // Убираем класс отображения
